	...
	const onSubmit = async (e) => {
		...
		const storeImage = async(image) => {
			return new Promise((resolve, reject) => {
				...
				const storage = getStorage()
				const fileName = `${auth.currentUser.uid}-${image.name.replace(/\.[^.$]+$/, '')}-${uuidv4()}`
		...
		//location && (formDataCopy.location = location) // sometimes location from GoogleMap does not work properly
